...
public class ProfilePageController {
  ...
  @FXML
  private PasswordField currentPasswordInput, newPasswordInput, repeatNewPasswordInput;
  ...
  /**
   * Sets the bikeRentalManager when the view is changed from main menu to profile page
   * 
  ...
  /**
   * Takes the input strings from the fields in the change-user-password-pane, validates them,
   * and sets the new, given password if the validation succeeds. If the validation does not
  ...
  // ----------- Metode for å gå tilbake til hovedvinduet ----------------
    ...
    /**
     * Sets the view to main menu, keeping the bikeRentalManager object (ensures that the user is still logged in)
     */
    @FXML
    void backToMainMenu() {
      try {
        ((Stage) usernameTitle.getScene().getWindow()).setScene(getMainMenuScene(this.bikeRentalManager));
      } catch (Exception e) {
   ...
   * 
   * @param bikeRentalManager the bikeRentalManager currently in use. Should be {@code null} if user should be logged out.
  ...
  //TODO: Metode for å oppdatere utleiehistorikk (om vi velger å implementere det):
