...
/**
 * Class for {@code Place} objects. Keeps track of the name and
 * maximumNumberOfBikes, as well as a list of {@code Bike} objects.
 */
public class Place implements Iterable<Bike> {
    ...
    /**
     * Constucts a {@code Place} object used to store {@code Bike} objects at a
     * specific location.
     *
     * @param name                 the String name of the {@code Place} object
     * @param maximumNumberOfBikes the maximum number of bikes allowed in the
     *                             {@code Place} object.
     * @throws IllegalArguemtException if name or maximumNumberOfBikes are not valid
     *                                 according to
     *                                 the correspinding validators in the
     *                                 {@code Place} class.
     */
     ...
     * @param maximumNumberOfBikes the maximum number of bikes allowed in the
     *                             {@code Place} object.
     * @param bikes                a list of {@code Bike} objects.
     * @throws IllegalArguemtException if name or maximumNumberOfBikes are not valid
     *                                 according to
     *                                 the correspinding validators in the
     *                                 {@code Place} class, or if bikes is null.
     * @throws IllegalStateException   if the number of {@code Bike} objects in
     *                                 input bikes is above
     *                                 the limit given in input
    ...
    /**
     * Getter for a copy of bike-list of the {@code Place} object, which contains
     * all the {@code Bike}
    ...
    /**
     * Removes the {@code Bike} object with the given bikeID from the {@code Place}
     * object, and
     * returns it.
     *
     * @param bikeId the id String of the {@code Bike} object to remove
     * @return the {@code Bike} object that has been removed from the {@code Place}
     *         object.
     * @throws IllegalArgumentException if the bikeID input is not a valid bikeID
     *                                  according to
     *                                  the validateID method in the {@code Place}
     *                                  class.
     */
    public Bike removeAndGetBike(String bikeId) {
        validateId(bikeId);
        Bike bikeToRemove = this.bikes.stream().filter(
                bike -> bike.getId().equals(bikeId)).findFirst()
                .orElseThrow(() -> new IllegalArgumentException(
                        "Det finnes ingen sykkel her med denne ID-en."));
        this.bikes.remove(bikeToRemove);
        return bikeToRemove;
    }

    /**
     * Adds the given {@code Bike} object to the {@code Place} object's list of
     * bikes.
     *
     * @param bike the {@code Bike} object to add
     * @throws IllegalStateException    if the size of the {@code Place} object's
     *                                  list of bikes is equal
     *                                  to the object's maximumNumberOfBikes before
     *                                  adding the given {@code Bike} object.
    ...
    public void addBike(Bike bike) {
        if (this.bikes.size() == this.maximumNumberOfBikes) {
            throw new IllegalStateException(
                    "Stedet er fullt, sykkelen må plasseres på et annet sted.");
        }
     ...
     * @throws IllegalArgumentException if the iD is null or the iD String doesn't
     *                                  consist
     *                                  of only numerals and/or capital letters,
     *                                  and/or has a length not equal to 8.
     */
    private void validateId(String id) {
        this.inputNotNullValidator(id);
        if (!Pattern.matches("[A-Z0-9]{8}", id)) {
            throw new IllegalArgumentException(
                    "ID-format ugyldig. Må bestå av tall og"
                            + "store bokstaver og ha en lengde på 8 tegn.");
        }
    }
